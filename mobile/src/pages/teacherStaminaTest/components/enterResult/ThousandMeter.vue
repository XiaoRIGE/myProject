...
<script>
import { thousandMeter } from './section';
export default {
  name: 'ThousandMeter',
  props: {
    type: {
  ...
  computed: {
    formatDate() {
      ...
      if (this.value.length <= 2) return `${this.value}''`;
        ...
        const m = this.value.slice(0, 1);
        const M = this.value.slice(1);
        return `${m}' ${M}''`;
      }
    }
  },
    ...
    compInfo: {
      ...
      handler(newval) {
        if (newval.fitnessData) {
          // 将秒转为分秒
          if (!newval.fitnessData) return;
          if (newval.fitnessData < 60) {
            this.value = newval.fitnessData.toString();
          } else {
            const M = Math.floor(newval.fitnessData / 60); //  分
            const s = newval.fitnessData % 60; // 秒
            this.value = `${M}${s > 9 ? s : `0${s}`}`;
          }
        }
      }
    }
  ...
  methods: {
    ...
    calculationResult() {
      ...
      const formatDate = thousandMeter.formatDate;
      if (sex === 1) {
        // 男
        // 等级
        const levelSectionsMan = thousandMeter['levelSectionsMan'][grade];
        // 分数区间
        const scoreSections = thousandMeter['scoreSectionsMan'][grade];
        //  等级
        for (let i = 0; i < levelSectionsMan.length; i++) {
          const { start, end, level } = levelSectionsMan[i];
          if (
            formatDate(value) >= formatDate(start) &&
            formatDate(value) < formatDate(end)
          ) {
            result.level = level;
            break;
          }
      ...
      if (sex === 2) {
        // 女
        // 等级
        const levelSectionsWoman = thousandMeter['levelSectionsWoman'][grade];
        // 分数区间
        const scoreSections = thousandMeter['scoreSectionsWoman'][grade];
    ...
    validateCurrentFile(value) {
      // 范围
      this.rules.valid = value >= 120 && value <= 959;
      ...
      // 验证格式-判断后2位数是否大于60
      if (value.length === 3) {
        const endNumber = value.slice(1, value.length);
        if (endNumber && endNumber > 59) {
          this.rules.message = '输入格式不正确';
    ...
    validate(cb) {
      ...
      if (this.type === 't10') {
        // 800米
        query.data.eightHundredMeters = thousandMeter.formatDate(this.value);
      } else if (this.type === 't8') {
        // 1000 m
        query.data.thousandMeters = thousandMeter.formatDate(this.value);
      }
    ...
    keyChange(value) {
      ...
      // 首位不能为0
      if ((value === '0' || value === 0) && this.value.length === 0) return;
      // 判断范围  120 && value <= 959
