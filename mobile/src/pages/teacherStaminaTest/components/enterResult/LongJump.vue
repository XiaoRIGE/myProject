  ...
  methods: {
    ...
    calculationResult() {
      ...
      if (sex === 1) {
        // 男
        // 等级
        const levelSectionsMan = longJump['levelSectionsMan'][grade];
        // 分数区间
        const scoreSections = longJump['scoreSectionsMan'][grade];
      ...
      if (sex === 2) {
        // 女
        // 等级
        const levelSectionsWoman = longJump['levelSectionsWoman'][grade];
        // 分数区间
        const scoreSections = longJump['scoreSectionsWoman'][grade];
    ...
    validateCurrentFile(value) {
      // 范围
      this.rules.valid = value >= 50 && value <= 400;
