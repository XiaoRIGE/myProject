...
<script>
import { fiftyMeter } from './section';
export default {
  name: 'FiftyMeter',
  props: {
    compInfo: {
  ...
  methods: {
    ...
    calculationResult() {
      ...
      if (sex === 1) {
        // 男
        // 等级
        const levelSectionsMan = fiftyMeter['levelSectionsMan'][grade];
        // 分数区间
        const scoreSections = fiftyMeter['scoreSectionsMan'][grade];
      ...
      if (sex === 2) {
        // 女
        // 等级
        const levelSectionsWoman = fiftyMeter['levelSectionsWoman'][grade];
        // 分数区间
        const scoreSections = fiftyMeter['scoreSectionsWoman'][grade];
    ...
    validateCurrentFile(value) {
      // 范围
      this.rules.valid = value >= 5 && value <= 30;
    ...
    keyChange(value) {
      ...
      // 只能保留一位小数
      const oldVal = this.value.toString() + value;
