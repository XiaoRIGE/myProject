...
<script>
import { vitalCapacity } from './section';
export default {
  name: 'VitalCapacity',
  props: {
    compInfo: {
  ...
  methods: {
    ...
    calculationResult() {
      ...
      if (sex === 1) {
        // 男
        // 等级
        const levelSectionsMan = vitalCapacity['levelSectionsMan'][grade];
        // 分数区间
        const scoreSections = vitalCapacity['scoreSectionsMan'][grade];
      ...
      if (sex === 2) {
        // 女
        // 等级
        const levelSectionsWoman = vitalCapacity['levelSectionsWoman'][grade];
        // 分数区间
        const scoreSections = vitalCapacity['scoreSectionsWoman'][grade];
    ...
    validateCurrentFile(value) {
      // 范围
      this.rules.valid = value >= 500 && value <= 9999;
