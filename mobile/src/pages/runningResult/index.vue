<template>
  <view class="running-result" :style="{ height: screenHeight + 'px' }">
    <map
      v-if="showMap"
      id="maps"
      ref="maps"
    ...
    >
      ...
      <cover-image
        ...
        @click="back"
      />
      <cover-view class="nav-title" :style="[{ top: StatusBar + 10 + 'px' }]">
        <text class="nav-title-text">跑步轨迹</text>
      </cover-view>
      <view
        class="running-record"
        :style="{ height: runningInfo.page !== 'show' ? '340rpx' : '240rpx' }"
      >
        <view class="running-record-top">
          <view class="item">
            <image
              class="record-img"
              src="/static/img/runningResult/running-speed.png"
            >
          ...
          <view class="item">
            <image
              class="record-img"
              src="/static/img/runningResult/running-time.png"
            >
          ...
          <view class="item">
            <image
              class="record-img"
              src="/static/img/runningResult/running-step.png"
            >
        ...
        <view
          v-if="runningInfo.page !== 'show'"
          class="running-record-bottom"
          @click="appealClick"
        >
          <image
            class="query-img"
            src="~@/static/img/runningResult/running-query.png"
          />
          <text v-if="appealState === null" class="hint">我要申诉</text>
          <text v-if="appealState === 0" class="hint">已申诉，查看进度</text>
          <text v-if="appealState === 1" class="hint">申诉成功，查看详情</text>
          <text v-if="appealState === 2" class="hint">申诉失败，查看详情</text>
        </view>
      </view>
      <view class="running-info">
        <view class="running-info-top">
          <view class="top-left">
            <image
              class="type-img"
              src="/static/img/runningResult/range-running.png"
            />
        ...
        <view class="running-info-center">
          ...
          <view class="center-left">
            <view class="left-top">
              <text v-if="data && data.detailsRoutineVO" class="energy-count">
                {{ data.detailsRoutineVO.calorie || 0 }}
              </text>
        ...
        <view class="running-info-bottom">
          <view
            class="reason"
            :style="
              data && data.uneffectiveReason ? {} : { visibility: 'hidden' }
            "
            @click="checkReason(data.uneffectiveReason)"
          >
            <image
              class="hint-img"
              src="~@/static/img/runningResult/running-hint.png"
            />
            <text class="hint-text">查看原因</text>
          </view>
          <view v-if="data" class="number">
            <view>
              <text class="number-text">
                {{
                  data.scoryType === 1 ? '本次计入有效里程' : '本次计入有效段数'
                }}
              </text>
            </view>
            <view>
              <!-- 里程模式 -->
              <text v-if="data.scoryType === 1" class="number-count">
                {{
                  data.operationMileage === null
                    ? data.effectiveMileage
                    : data.operationMileage
                }}
              </text>
              <!-- 段数模式 -->
              <text v-if="data.scoryType === 2" class="number-count">
                {{
                  data.operationPart === null
                    ? data.effectivePart
                    : data.operationPart
                }}
              </text>
...
export default {
  data() {
    return {
      ...
      longitude: 0,
      avatarArr: [
        '/static/img/avatar/student-man-avatar.png',
        '/static/img/avatar/student-woman-avatar.png',
        '/static/img/avatar/teacher-man-avatar.png',
        '/static/img/avatar/teacher-woman-avatar.png'
      ],
      runningInfo: {},
      paceNumber: 0, //步数
      paceRange: 0, //步幅度,
  ...
  computed: {
    ...
    covers() {
      if (this.data) {
        const data = [
          {
            points: this.data.detailsRoutineVO.routineLine, //this.data.routineLine,
            color: '#FC6C37FF', //线的颜色
            width: 5 //线的宽度
          }
        ];
    ...
    markers() {
      if (this.data && this.data.detailsRoutineVO) {
        const markers = (this.data.detailsRoutineVO.signPoint || []).map(
          (item, index) => {
            return {
              id: index, //id 必填项
              width: 19,
    ...
    polygons() {
      if (
        this.data &&
        this.data.detailsRoutineVO &&
        this.data.detailsRoutineVO.effictiveRegion
      ) {
        return [
    ...
    avg() {
      if (this.data && this.data.detailsRoutineVO && this.data.keepTime) {
        return (
          this.data.detailsRoutineVO.paceNumber /
          (this.data.keepTime / 60)
        ).toFixed(2);
      }
      return 0;
    },
    ...
    avePace() {
      if (this.data && this.data.detailsRoutineVO) {
        const m = parseInt(
          (this.data.detailsRoutineVO.avePace % (1000 * 60 * 60)) / (1000 * 60)
        );
        let s = parseInt(
          (this.data.detailsRoutineVO.avePace % (1000 * 60)) / 1000
        );
        s = s < 10 ? `0${s}` : `${s}`;
        return `${m}'${s}''`;
      }
  ...
  methods: {
    ...
    async appealClick() {
      ...
      url += `?id=${
        state ? this.runningInfo.id : this.data.appealId
      }&page=detail`;
      if (state) {
  ...
  onLoad(option) {
    ...
    if (option.page === 'native') {
      ...
      // 原生跳转，直接跳转
      const state =
        option.appealState === null || option.appealState === 'null';
      let url = `/pages/runningAppeal/${state ? 'add' : 'detail'}`;
