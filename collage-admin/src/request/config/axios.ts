...
// 请求接口白名单（不需要设置Authorization）
const WhiteList = [
  '/authorization/organization/querySchool',
  '/authorization/user/manage/login',
  '/authorization/user/manage/resetPassword/mobileCode',
  '/authorization/user/manage/getMobileCode'
];
...
service.interceptors.request.use(async (config: AxiosRequestConfig) => {
  ...
  // 存在accessToken且当前接口不在白名单内，统一添加Authorization
  if (accessToken !== null && !WhiteList.includes(config.url as string)) {
